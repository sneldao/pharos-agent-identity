...
function resolveDeployment(): Deployment {
  for (const dep of Object.values(networksFile.deployment)) {
    if (dep.chainId === network.chainId) return dep;
  }
...
function isHexBytes32(s: string): s is Hex {
...
const server = new Server(
  { name: "pharos-identity", version: "0.1.0" },
  { capabilities: { tools: {} } }
);
      ...
      inputSchema: {
        ...
        properties: {
          tokenUri: {
            type: "string",
            description:
              "Optional metadata URI (IPFS CID, HTTPS URL, or empty). Stored on-chain as the token's metadata pointer.",
          },
          controller: {
            type: "string",
            description:
              "Optional controller address. If omitted, the caller's wallet becomes the controller.",
          },
        },
      },
    },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          capability: {
            type: "string",
            description:
              "Either a human-readable name (e.g. 'agent.commerce.escrow') or a 0x...bytes32 hash. Human names are keccak256-hashed internally.",
          },
          issuer: {
            type: "string",
            description:
      ...
      inputSchema: {
        ...
        properties: {
          ...
          capability: {
            type: "string",
            description:
              "Either a human-readable name (e.g. 'agent.commerce.escrow') or a 0x...bytes32 hash.",
          },
          nonce: {
            type: "string",
          ...
          issuerKey: {
            type: "string",
            description:
              "Optional. Issuer's private key. If provided, used to sign the revoke tx. If omitted, the caller's $PRIVATE_KEY is used.",
          },
        },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          expiresInSeconds: {
            type: "number",
            description:
              "Optional. Seconds from now until expiry. Default 2,592,000 (30 days).",
          },
        },
