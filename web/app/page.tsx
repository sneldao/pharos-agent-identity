...
export default async function HomePage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-8 pt-12 pb-24 sm:pt-20 sm:pb-32">
      <header className="flex items-baseline justify-between text-xs">
        <p className="eyebrow">Ligis · index 00</p>
      ...
      <section className="mt-24 sm:mt-32">
        ...
        <p className="mt-12 max-w-2xl font-serif text-xl leading-relaxed text-ink-soft">
          Ligis issues portable identity and verifiable credentials on Pharos.
          Two contracts compose. Anyone can sign. Anyone can verify. There is no
          administrator and no key the foundation could lose.
        </p>
        <p className="mt-6 max-w-2xl font-serif text-base italic leading-relaxed text-ink-quiet">
          {stats.ok ? (
            <>
              ...
              index, counted at block{" "}
              <span className="font-mono not-italic tabular text-ink">
                {Number(stats.block).toLocaleString("en")}
              </span>{" "}
              on {network.name.toLowerCase()}.
            </>
          ) : (
            <>The index is presently unreachable. {stats.error}</>
          )}
        </p>
      </section>
